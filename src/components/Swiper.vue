    ...
    <div class="swiper-wrapper">
      ...
      <template v-if="direction == 'right'">
        ...
        <v-list-tile class="swiper-slide">
          <v-list-tile-content>
            <v-list-tile-title>{{ `${item} - Swipe me to ${direction}` }}</v-list-tile-title>
          </v-list-tile-content>
          <v-list-tile-action>
      ...
      <template v-if="direction == 'left'">
        <v-list-tile class="swiper-slide">
          ...
          <v-list-tile-content>
            <v-list-tile-title class="text-xs-right">{{ `Swipe me to ${direction} - ${item}` }}</v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
        <v-list-tile class="swiper-slide error"></v-list-tile>
      </template>
...
export default {
  ...
  mounted() {
    ...
    swiper.on("transitionEnd", function() {
      ...
      if (
        (direction === "right" && activeIndex === 0) ||
        (direction === "left" && activeIndex === 1)
      ) {
